...
const UnlockedButton = ({
  ...
  const ANIMATE = useMemo(() => {
    return {
      NEW_PROJECT_PARENT: { x: isHovering ? 2 : 0 },
      NEW_PROJECT_CHILD: { rotate: isHovering ? [0, 90, 0] : 0 },
    } as const;
  }, [isHovering]);
  ...
  const hoverTransition = (i: number) =>
    useMemo(
      () =>
        ({
          duration: 0.8,
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        animate={ANIMATE.NEW_PROJECT_PARENT}
        transition={TRANSITION.NEW_PROJECT_PARENT}
      >
        <motion.div
          animate={ANIMATE.NEW_PROJECT_CHILD}
